...
- M10 cluster tools: complete through its acceptance boundary. Singleton and
  ...
  routing, best-effort fan-out, and ordered per-target failure reports. The
  end-to-end remote boundary now also hard-gates fixed status/delta/publish/path
  manifests and serializer ids, nested business-message metadata, canonical
  recipient validation, typed mediator re-entry, and rejection of local-only
  ...
  The singleton remote boundary now hard-gates its five stable serializer ids,
  exact-incarnation handover payloads, canonical system recipient validation,
  ordered non-transactional effect batches, local-effect rejection, generic
...
whose builder registers typed business protocols before bind; its manifest
router rejects duplicate or unknown protocols and retains type erasure only at
the decoded-envelope dispatch boundary. A loopback integration test delivers
...
`GossipStatus`, `Leave`, `Down`, and `ExitingConfirmed` have stable manifests,
...
- Latest M13 validation refresh after coordinated-shutdown phase timeout
  ...
  ```bash
  cargo test -p kairo-actor non_recovering_phase_reports_timeout --all-targets --all-features -- --nocapture
  cargo test -p kairo-actor --all-targets --all-features
  cargo fmt --all -- --check
  cargo clippy -p kairo-actor --all-targets --all-features -- -D warnings
  git diff --check
...
- Latest M13 validation refresh after coordinated-shutdown same-phase late
  ...
  ```bash
  cargo test -p kairo-actor coordinated_shutdown_same_phase_task_added_during_run_is_too_late --all-targets --all-features -- --nocapture
  cargo test -p kairo-actor --all-targets --all-features
  cargo fmt --all -- --check
  cargo clippy -p kairo-actor --all-targets --all-features -- -D warnings
  git diff --check
...
- Latest M13 validation refresh after direct actor-stop timer cleanup parity
  ...
  ```bash
  cargo test -p kairo-actor direct_actor_stop_cancels --all-targets --all-features -- --nocapture
  cargo test -p kairo-actor --all-targets --all-features
  cargo fmt --all -- --check
  cargo clippy -p kairo-actor --all-targets --all-features -- -D warnings
  git diff --check
...
- Latest M13 validation refresh after direct actor-stop receive-timeout
  ...
  ```bash
  cargo test -p kairo-actor active_receive_timeout --all-targets --all-features -- --nocapture
  cargo test -p kairo-actor --all-targets --all-features
  cargo fmt --all -- --check
  cargo clippy -p kairo-actor --all-targets --all-features -- -D warnings
  git diff --check
...
- Latest M13 validation refresh after cluster-tools TCP peer-runtime late-route
  shutdown cleanup:

  ```bash
  cargo test -p kairo-cluster-tools peer_runtime_shutdown_clears_late_routes_registered_during_shutdown --all-targets --all-features -- --nocapture
  cargo test -p kairo-cluster-tools --all-targets --all-features
  cargo fmt --all
  cargo fmt --all -- --check
  cargo clippy -p kairo-cluster-tools --all-targets --all-features -- -D warnings
...
- Latest M13 validation refresh after cluster TCP peer-runtime late-route
  shutdown cleanup:

  ```bash
  cargo test -p kairo-cluster peer_runtime_shutdown_clears_late_routes_registered_during_shutdown --all-targets --all-features -- --nocapture
  cargo test -p kairo-cluster --all-targets --all-features
  cargo fmt --all
  cargo fmt --all -- --check
  cargo clippy -p kairo-cluster --all-targets --all-features -- -D warnings
...
- Latest M13 validation refresh after distributed-data and cluster-tools TCP
  connector late-route shutdown cleanup:

  ```bash
  cargo test -p kairo-distributed-data connector_stop_clears_late_routes_registered_during_shutdown --all-targets --all-features -- --nocapture
  cargo test -p kairo-cluster-tools connector_stop_clears_late_routes_registered_during_shutdown --all-targets --all-features -- --nocapture
  cargo test -p kairo-distributed-data --all-targets --all-features
  cargo test -p kairo-cluster-tools --all-targets --all-features
  cargo fmt --all
  cargo fmt --all -- --check
...
- Latest M13 validation refresh after cluster and cluster-tools TCP bootstrap
  coordinated-shutdown late-route cleanup:

  ```bash
  cargo test -p kairo-cluster bootstrap_coordinated_shutdown_clears_late_route_registered_during_connector_stop --all-targets --all-features -- --nocapture
  cargo test -p kairo-cluster-tools bootstrap_coordinated_shutdown_clears_late_route_registered_during_connector_stop --all-targets --all-features -- --nocapture
  cargo test -p kairo-cluster --all-targets --all-features
  cargo test -p kairo-cluster-tools --all-targets --all-features
  cargo fmt --all
  cargo fmt --all -- --check
...
- Latest M13 validation refresh after sharding region death-watch restart
  status audit:
...
- Escalated parent-stop coverage now also pins recursive child-tree teardown:
  a parent stopped by child failure escalation does not terminate or notify
  watchers until a descendant grandchild that is blocked in `PostStop` has fully
  terminated.
...
- Distributed-data TCP peer connector lifecycle coverage now also pins the
  pending-reconnect stop boundary: a connector stopped while a failed dial is
  awaiting retry clears its owned retry state, unsubscribes from cluster
  events, and does not install a route when the peer later becomes dialable.
...
- Distributed-data TCP peer bootstrap lifecycle coverage now proves the
  bootstrap-registered coordinated-shutdown stop task unsubscribes its
  connector from later membership changes: after a live route is cleared by
  the bootstrap shutdown task, a follow-up membership publication produces no
  stale adapter dead letters and does not recreate the distributed-data socket
  route.
- Cluster-tools TCP peer bootstrap lifecycle coverage now proves the
  bootstrap-registered coordinated-shutdown stop task unsubscribes its
  connector from later membership changes: after a live pubsub/singleton route
...
- Local `Listing<M>` now exposes `all_service_instances` and
  ...
  always true, matching Pekko's non-clustered receptionist semantics.
...
- Child-preserving parent restart now asks restartable surviving children to
...
- Manual scheduler advancement now runs due tasks at their scheduled manual
  timestamps before moving to the final target time, so nested schedules
  created by a due task use the due task's time rather than the caller's
  eventual `advance` target.
...
- Distributed-data TCP peer bootstrap now also validates the higher-level
  `ActorSystem::run_coordinated_shutdown` facade after a live
  membership-derived route is installed, proving the registered connector stop
  task clears the sender association cache before system termination.
...
- `kairo-cluster-tools` TCP peer-runtime shutdown coverage now also pins the
  late-route cleanup invariant at the composed runtime layer: after tracked
  peer routes are removed and a close hook registers a fresh cached
  association route, shutdown delegates to the association runtime and returns
  with the association cache empty.
- `kairo-cluster` TCP peer-runtime shutdown coverage now pins the same
...
- Distributed-data and cluster-tools actor-backed TCP peer connector coverage
  now pins the same late-route shutdown invariant at their actor-owned runtime
  boundaries, so all three TCP peer connector families now reject
  close-hook-created association route leaks during connector stop.
- Distributed-data and cluster-tools actor-backed TCP peer connector coverage
...
- Cluster and cluster-tools TCP peer bootstrap coverage now pin the same
  late-route shutdown invariant through their public coordinated-shutdown
  facades, so all three TCP bootstrap families assert close-hook-created
  association routes are gone before their connector stop task reports success.
...
- `kairo-cluster` actor-backed TCP peer connector coverage now also pins the
  mixed membership-shrink case: when one membership route is active and another
  peer is pending reconnect, removing the pending peer clears only the
  reconnect state while the surviving route still delivers a stable-manifest
  `Join`.
- `kairo-distributed-data` actor-backed TCP peer connector coverage now also
  pins the mixed membership-shrink case: when one replicator route is active
  and another peer is pending reconnect, removing the pending peer clears only
  the reconnect state while the surviving route still delivers a
  stable-manifest `ReplicatorRead`.
...
- `kairo-cluster` TCP bootstrap coverage now exercises a mixed active-route
  ...
  messages.
- `kairo-cluster` TCP bootstrap coverage now drives local self-removal through
  the public bootstrap facade: when reduced gossip removes the bootstrap node
  itself, the connector clears the installed peer route, the association cache
  reaches zero routes, and stale membership delivery through the old outbound
  route is rejected.
...
- `kairo-actor` actor-system termination coverage now pins the same queued
...
- `kairo-actor` actor-system termination coverage now also pins stashed-message
...
- `kairo-cluster-sharding` shard-region death-watch coverage has been
  re-audited against Pekko `ShardRegion.receiveTerminated`: plain local shard
  termination removes the shard without restart, while local and shared
...
- `kairo-actor` direct actor-stop receive-timeout coverage now pins the same
  Pekko-style cleanup at the actor boundary for `/user` and `/system` actors:
  stopping an actor with an armed receive timeout cancels the scheduled
  timeout, so advancing manual time does not deliver the timeout or publish
  late dead letters.
- `kairo-actor` direct actor-stop timer coverage now pins the same cleanup
  parity for `/user` and `/system` actors: externally stopping an actor with
  an active owner timer cancels the scheduled timer without delivering the
  timer callback or publishing late dead letters after manual time advances.
...
```bash
cargo test -p kairo-cluster cluster_daemon_ --all-targets --all-features
cargo test -p kairo-cluster cluster_system_manifests_are_stable --all-targets --all-features
cargo test -p kairo-cluster --all-targets --all-features
cargo fmt --all -- --check
cargo clippy -p kairo-cluster --all-targets --all-features -- -D warnings
git diff --check
...
```bash
cargo test -p kairo-cluster bootstrap_clears_peer_routes_when_self_member_is_removed --all-targets --all-features -- --nocapture
cargo test -p kairo-cluster --all-targets --all-features
cargo fmt --all
cargo fmt --all -- --check
cargo clippy -p kairo-cluster --all-targets --all-features -- -D warnings
...
```bash
cargo test -p kairo-actor escalated_parent_stop_waits_for_descendant_grandchild_before_notifying_watchers --all-targets --all-features -- --nocapture
cargo test -p kairo-actor --all-targets --all-features
cargo fmt --all -- --check
cargo clippy --workspace --all-targets --all-features -- -D warnings
git diff --check
...
cargo fmt --all
cargo clippy -p kairo-distributed-data --all-targets --all-features -- -D warnings
cargo test -p kairo-cluster peer_runtime_preserves_successful_routes_when_later_snapshot_dial_fails --all-targets --all-features -- --nocapture
cargo test -p kairo-cluster --all-targets --all-features
cargo fmt --all
cargo clippy -p kairo-cluster --all-targets --all-features -- -D warnings
...
cargo clippy -p kairo-cluster --all-targets --all-features -- -D warnings
git diff --check
cargo test -p kairo-distributed-data bootstrap_two_nodes_install_peer_routes_from_cluster_membership --all-targets --all-features
cargo test -p kairo-distributed-data --all-targets --all-features
cargo fmt --all -- --check
cargo clippy -p kairo-distributed-data --all-targets --all-features -- -D warnings
git diff --check
...
cargo clippy -p kairo-serialization --all-targets --all-features -- -D warnings
git diff --check
cargo test -p kairo-testkit actor_harness_watch_subject_observes_subject_stop --all-targets --all-features
cargo test -p kairo-testkit actor_harness --all-targets --all-features
cargo test -p kairo-testkit --all-targets --all-features
cargo test -p kairo-testkit --doc --all-features
cargo fmt --all -- --check
...
cargo clippy -p kairo-testkit --all-targets --all-features -- -D warnings
git diff --check
cargo test -p kairo-actor manual_scheduler_runs_nested_work_from_due_task_time --all-targets --all-features -- --nocapture
cargo test -p kairo-actor scheduler --all-targets --all-features
cargo test -p kairo-testkit manual_time --all-targets --all-features
cargo test -p kairo-actor --all-targets --all-features
cargo test -p kairo-testkit --all-targets --all-features
...
cargo test -p kairo-distributed-data connector_clear_routes_removes_multiple_active_peer_routes --all-targets --all-features -- --nocapture
...
cargo test -p kairo-cluster-tools connector_clear_routes_removes_multiple_active_peer_routes --all-targets --all-features -- --nocapture
cargo test -p kairo-cluster tcp_peer_runtime --all-targets --all-features
cargo test -p kairo-cluster-tools tcp_peer_runtime --all-targets --all-features
cargo test -p kairo-cluster --all-targets --all-features
cargo test -p kairo-cluster-tools --all-targets --all-features
...
git diff --check
cargo test -p kairo public_docs_document_m13_validation_gates --all-targets --all-features
cargo test -p kairo-cluster-sharding region_coordinator_discovery_reports_remote_target_without_local_registration --all-targets --all-features
...
cargo clippy -p kairo-actor --all-targets --all-features -- -D warnings
git diff --check
cargo test -p kairo-distributed-data connector_keeps_route_and_clears_pending_reconnect_when_peer_leaves_membership --all-targets --all-features -- --nocapture
cargo test -p kairo-distributed-data --all-targets --all-features
cargo fmt --all
cargo fmt --all -- --check
cargo clippy -p kairo-distributed-data --all-targets --all-features -- -D warnings
git diff --check
cargo test -p kairo-cluster connector_keeps_route_and_clears_pending_reconnect_when_peer_leaves_membership --all-targets --all-features -- --nocapture
cargo test -p kairo-cluster --all-targets --all-features
cargo fmt --all -- --check
cargo clippy -p kairo-cluster --all-targets --all-features -- -D warnings
git diff --check
