...
use crate::{
    ...
    HeartbeatRemoteResponseInbound, HeartbeatSenderMsg, Join, Member, MemberEvent, MemberStatus,
    Reachability, ReachabilityEvent, register_cluster_protocol_codecs,
...
fn peer_runtime_replaces_routes_on_reachability_changed_self_observer_set() {
    let _guard = cluster_socket_test_lock();
    let sender_kit =
        ActorSystemTestKit::new("cluster-peer-runtime-reachability-change-sender").unwrap();
    let first_kit =
        ActorSystemTestKit::new("cluster-peer-runtime-reachability-change-first").unwrap();
    let second_kit =
        ActorSystemTestKit::new("cluster-peer-runtime-reachability-change-second").unwrap();
    let registry = registry();
    ...
    let second_node = second.self_node().clone();
    let observer = node("observer", 2660, 4);
    let outbound = Arc::new(sender.association_cache().clone()) as Arc<dyn RemoteOutbound>;
    let first_membership_outbound = ClusterMembershipWireOutbound::new(
        first_node.clone(),
        registry.clone(),
        ClusterMembershipRemoteEnvelopeOutbound::from_arc(outbound.clone()),
    );
    ...
    let first_error = first_membership_outbound
        .send_membership(ClusterMembershipMsg::Join {
            join: Join {
                node: sender_node.clone(),
    ...
    let second_error = second_membership_outbound
        .send_membership(ClusterMembershipMsg::Join {
            join: Join {
                node: sender_node,
...
fn peer_runtime_clears_routes_when_self_member_is_removed() {
    ...
    let receiver_kit =
        ActorSystemTestKit::new("cluster-peer-runtime-self-remove-receiver").unwrap();
    let registry = registry();
...
fn peer_runtime_shutdown_clears_late_routes_registered_during_shutdown() {
    let _guard = cluster_socket_test_lock();
    let sender_kit = ActorSystemTestKit::new("cluster-peer-runtime-late-route-sender").unwrap();
    let receiver_kit = ActorSystemTestKit::new("cluster-peer-runtime-late-route-receiver").unwrap();
    let registry = registry();
    let mut sender = bind_peer_runtime("sender", 1, 11, &sender_kit, registry.clone());
    let (receiver, _receiver_probes) =
        bind_association_runtime_with_probes("receiver", 2, 22, &receiver_kit, registry);
    let sender_node = sender.self_node().clone();
    let receiver_node = receiver.self_node().clone();
...
fn peer_runtime_shutdown_clears_multiple_active_peer_routes() {
    let _guard = cluster_socket_test_lock();
    let sender_kit = ActorSystemTestKit::new("cluster-peer-runtime-multi-shutdown-sender").unwrap();
    let second_kit = ActorSystemTestKit::new("cluster-peer-runtime-multi-shutdown-second").unwrap();
    let third_kit = ActorSystemTestKit::new("cluster-peer-runtime-multi-shutdown-third").unwrap();
    let registry = registry();
    let mut sender = bind_peer_runtime(
...
fn peer_runtime_preserves_successful_routes_when_later_snapshot_dial_fails() {
    let _guard = cluster_socket_test_lock();
    let sender_kit = ActorSystemTestKit::new("cluster-peer-runtime-partial-sender").unwrap();
    let bound_kit = ActorSystemTestKit::new("cluster-peer-runtime-partial-bound").unwrap();
    let missing_kit = ActorSystemTestKit::new("cluster-peer-runtime-partial-missing").unwrap();
    let registry = registry();
...
fn peer_runtime_clear_pending_reconnects_preserves_active_routes() {
    let _guard = cluster_socket_test_lock();
    let sender_kit = ActorSystemTestKit::new("cluster-peer-runtime-clear-pending-sender").unwrap();
    let bound_kit = ActorSystemTestKit::new("cluster-peer-runtime-clear-pending-bound").unwrap();
    let registry = registry();
    let bound_port = unused_port();
    let missing_port = unused_port();
