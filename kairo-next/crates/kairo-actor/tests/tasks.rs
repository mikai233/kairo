...
#[test]
fn spawn_task_send_is_rejected_after_owner_stop() {
    let system = ActorSystem::builder("tasks").build().unwrap();
    let (pre_restart_tx, _pre_restart_rx) = mpsc::channel();
    let actor = system
        .spawn(
            "task-owner",
    ...
    actor
        .tell(ScopedTaskMsg::StartAndStop {
            release: release_rx,
...
#[test]
fn pipe_to_self_completion_is_rejected_after_owner_stop() {
    let system = ActorSystem::builder("tasks").build().unwrap();
    let (pre_restart_tx, _pre_restart_rx) = mpsc::channel();
    let actor = system
        .spawn(
    ...
    actor
        .tell(ScopedTaskMsg::PipeAndStop {
            release: release_rx,
            stale_delivered: stale_delivered_tx,
        })
        .unwrap();
    ...
    assert_eq!(dead_letters[0].reason(), "actor is stopped");
    assert!(
        stale_delivered_rx
            .recv_timeout(Duration::from_millis(100))
            .is_err()
