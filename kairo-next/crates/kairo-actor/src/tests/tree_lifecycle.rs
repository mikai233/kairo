...
impl Actor for ChildStoppingParent {
    ...
    fn receive(&mut self, ctx: &mut Context<Self::Msg>, msg: Self::Msg) -> ActorResult {
        match msg {
            ...
            ChildStopMsg::Block { entered, release } => {
                entered
                    .send(())
                    .map_err(|error| ActorError::Message(error.to_string()))?;
                release
...
impl Actor for BlockingReceiveParent {
    ...
    fn receive(&mut self, ctx: &mut Context<Self::Msg>, msg: Self::Msg) -> ActorResult {
        match msg {
            ...
            BlockingReceiveParentMsg::Ping(reply_to) => {
                reply_to
                    .send("alive")
                    .map_err(|error| ActorError::Message(error.to_string()))?;
            }
...
enum EscalatingStopMsg {
    Fail,
}

struct EscalatingStopChild;

impl Actor for EscalatingStopChild {
    type Msg = EscalatingStopMsg;

    fn receive(&mut self, _ctx: &mut Context<Self::Msg>, msg: Self::Msg) -> ActorResult {
        match msg {
            EscalatingStopMsg::Fail => Err(ActorError::Message("escalate parent".to_string())),
        }
    }
}

struct EscalatedRecursiveStopParent {
    failing_child: mpsc::Sender<ActorRef<EscalatingStopMsg>>,
    child_path: mpsc::Sender<ActorPath>,
    grandchild_path: mpsc::Sender<ActorPath>,
    grandchild_entered_stop: mpsc::Sender<()>,
    grandchild_release_stop: Option<mpsc::Receiver<()>>,
...
impl Actor for EscalatedRecursiveStopParent {
    type Msg = ChildStopMsg;

    fn started(&mut self, ctx: &mut Context<Self::Msg>) -> ActorResult {
        let failing_child = ctx.spawn(
            "failing-child",
            Props::new(|| EscalatingStopChild).with_supervisor(SupervisorStrategy::Escalate),
        )?;
        self.failing_child
            .send(failing_child)
            .map_err(|error| ActorError::Message(error.to_string()))?;
...
#[test]
fn preserving_restart_does_not_wait_for_live_blocked_child_receive() {
    let system = ActorSystem::builder("test").build().unwrap();
    let parent = system
        .spawn(
            "parent",
    ...
    fail_ack_rx.recv_timeout(Duration::from_secs(1)).unwrap();
    parent
        .tell(BlockingReceiveParentMsg::Ping(ping_tx))
        .unwrap();

    assert_eq!(
        ping_rx.recv_timeout(Duration::from_secs(1)).unwrap(),
        "alive",
        "preserving restart must not wait for a live child receive turn to finish"
    );
    assert!(!child.is_stopped());
    block_release_tx.send(()).unwrap();
    assert!(system.dead_letters().is_empty());
...
#[test]
fn escalated_parent_stop_waits_for_descendant_grandchild_before_notifying_watchers() {
    let system = ActorSystem::builder("test").build().unwrap();
    let (failing_child_tx, failing_child_rx) = mpsc::channel();
    let (child_path_tx, child_path_rx) = mpsc::channel();
    let (grandchild_path_tx, grandchild_path_rx) = mpsc::channel();
    let (entered_stop_tx, entered_stop_rx) = mpsc::channel();
    let (release_stop_tx, release_stop_rx) = mpsc::channel();
    let parent = system
        .spawn(
            "parent",
            Props::new(move || EscalatedRecursiveStopParent {
                failing_child: failing_child_tx,
                child_path: child_path_tx,
                grandchild_path: grandchild_path_tx,
                grandchild_entered_stop: entered_stop_tx,
                grandchild_release_stop: Some(release_stop_rx),
        ...
        .unwrap();
    let failing_child = failing_child_rx
        .recv_timeout(Duration::from_secs(1))
        .unwrap();
    let child_path = child_path_rx.recv_timeout(Duration::from_secs(1)).unwrap();
    let grandchild_path = grandchild_path_rx
    ...
    failing_child.tell(EscalatingStopMsg::Fail).unwrap();
    assert!(failing_child.wait_for_stop(Duration::from_secs(1)));
    entered_stop_rx
        .recv_timeout(Duration::from_secs(1))
        .unwrap();
    ...
    assert!(
        !parent.wait_for_stop(Duration::from_millis(100)),
        "escalated parent stop must wait for descendant grandchild termination"
    );
    assert!(
        terminated_rx
            .recv_timeout(Duration::from_millis(100))
            .is_err(),
        "escalated parent termination must not notify watchers while a descendant is still stopping"
    );
...
#[test]
fn parent_stop_drains_queued_user_messages_before_waiting_for_children() {
    let system = ActorSystem::builder("test").build().unwrap();
    let parent = system
        .spawn("parent", Props::new(|| ChildStoppingParent { child: None }))
        .unwrap();
    ...
    assert!(
        ping_rx.recv_timeout(Duration::from_millis(100)).is_err(),
        "queued user message must not be delivered after a stop request"
    );
    assert!(
        system
            .dead_letters()
    ...
    assert!(
        !parent.wait_for_stop(Duration::from_millis(100)),
        "parent should still be waiting for child termination after draining queued messages"
    );
...
#[test]
fn actor_system_terminate_drains_queued_user_child_spawns_before_waiting_for_children() {
    let system = ActorSystem::builder("test").build().unwrap();
    let parent = system
        .spawn("parent", Props::new(|| ChildStoppingParent { child: None }))
        .unwrap();
    assert_system_terminate_drains_queued_child_spawns_before_waiting_for_children(system, parent);
}

#[test]
fn actor_system_terminate_drains_queued_system_child_spawns_before_waiting_for_children() {
    let system = ActorSystem::builder("test").build().unwrap();
    let parent = system
        .spawn_system(
            "system-parent",
...
fn assert_system_terminate_drains_queued_child_spawns_before_waiting_for_children(
    system: ActorSystem,
    parent: ActorRef<ChildStopMsg>,
) {
    let (child_entered_stop_tx, child_entered_stop_rx) = mpsc::channel();
    ...
    assert!(
        replacement_rx
            .recv_timeout(Duration::from_millis(100))
            .is_err(),
        "queued child spawn must not run after system termination starts"
    );
    assert!(
        system
            .dead_letters()
    ...
    assert!(
        records
            ...
            .filter(|record| record.recipient() == parent.path()
                && record.reason() == "actor is stopped"
                && record.message_type() == std::any::type_name::<ChildStopMsg>())
            .count()
            >= 2
    );
    assert!(
        terminated_rx
            .recv_timeout(Duration::from_millis(100))
            .is_err(),
        "system termination must still wait for child termination after draining queued child spawns"
    );
