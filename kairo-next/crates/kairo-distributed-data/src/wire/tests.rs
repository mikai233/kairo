...
fn wire_inbound_applies_delta_propagation_to_replicator_actor() {
    ...
    let replicator = system
        ...
        .unwrap();
    let (delta_reply, delta_rx) = probe::<DeltaPropagationReceiveReport>(&system, "delta-report");
    let (write_reply, _write_rx) = probe::<DirectWriteResult>(&system, "write-report");
    let (read_reply, _read_rx) = probe::<Result<DirectReadResult, String>>(&system, "read-report");
    let key = ReplicatorKey::new("counter");
...
fn wire_inbound_applies_write_and_serves_read() {
    ...
    let replicator = system
        .spawn("replicator", Props::new(ReplicatorActor::<GCounter>::new))
        .unwrap();
    let (delta_reply, _delta_rx) = probe::<DeltaPropagationReceiveReport>(&system, "delta-report");
    let (write_reply, write_rx) = probe::<DirectWriteResult>(&system, "write-report");
    let (read_reply, read_rx) = probe::<Result<DirectReadResult, String>>(&system, "read-report");
    let key = ReplicatorKey::new("counter");
    let inbound = ReplicatorWireInbound::new(
        replica("local"),
        registry.clone(),
