...
use kairo_remote::{
    RemoteAssociationAddress, RemoteAssociationCache, RemoteError, RemoteOutbound, RemoteSettings,
};
...
fn connector_serves_snapshots_while_runtime_command_is_blocked() {
    ...
    let kit = ActorSystemTestKit::new("ddata-tcp-peer-responsive").unwrap();
    ...
    let snapshots = kit
        .create_probe::<ReplicatorTcpPeerConnectorSnapshot>("responsive-snapshots")
        .unwrap();
    let started = Arc::new(std::sync::atomic::AtomicBool::new(false));
    let released = Arc::new(std::sync::atomic::AtomicBool::new(false));
    let gate = ReplicatorTcpPeerConnectorRuntimeCommandGate {
        ...
        .spawn(
            ...
            Props::new(move || {
                ReplicatorTcpPeerConnector::with_settings(
                    cluster,
                    runtime,
                    ReplicatorTcpPeerConnectorSettings::default().with_automatic_retry_ticks(false),
                )
...
fn connector_stop_does_not_wait_for_in_flight_runtime_command() {
    let _guard = ddata_socket_test_lock();
    let kit = ActorSystemTestKit::new("ddata-tcp-peer-stop-in-flight").unwrap();
    let runtime = bind_peer_runtime(
        "stop-in-flight",
        1,
        11,
...
fn connector_shutdown_waits_for_in_flight_command_cleanup_off_turn() {
    let _guard = ddata_socket_test_lock();
    let kit = ActorSystemTestKit::new("ddata-tcp-peer-shutdown-in-flight").unwrap();
    let runtime = bind_peer_runtime(
        "shutdown-in-flight",
        1,
        11,
    ...
    .unwrap();
    connector
        .tell(ReplicatorTcpPeerConnectorMsg::Shutdown {
...
fn connector_rejects_non_remote_member_without_pending_reconnect() {
    let _guard = ddata_socket_test_lock();
    let sender_kit = ActorSystemTestKit::new("ddata-tcp-peer-connector-local-only").unwrap();
    let retry_interval = Duration::from_millis(25);
    ...
    let cluster = Cluster::new(publisher.clone());
    let snapshots = sender_kit
        .create_probe::<ReplicatorTcpPeerConnectorSnapshot>("local-only-snapshots")
        .unwrap();
...
fn connector_keeps_route_and_clears_pending_reconnect_when_peer_leaves_membership() {
    let _guard = ddata_socket_test_lock();
    let sender_kit =
        ActorSystemTestKit::new("ddata-tcp-peer-connector-mixed-shrink-sender").unwrap();
    let bound_kit = ActorSystemTestKit::new("ddata-tcp-peer-connector-mixed-shrink-bound").unwrap();
    let retry_interval = Duration::from_millis(25);
    let registry = registry();
    let bound_port = unused_port();
    let missing_port = unused_port();
    ...
    let cluster = Cluster::new(publisher.clone());
    let snapshots = sender_kit
        .create_probe::<ReplicatorTcpPeerConnectorSnapshot>("mixed-shrink-snapshots")
        .unwrap();
    ...
    let bound_recipient = replicator_ref("mixed-shrink-bound", bound_port);
    let sender_ref = replicator_ref(
        sender_node.address.system(),
        sender_node.address.port().unwrap(),
    );
    ...
    assert_eq!(received.len(), 1);
    assert_eq!(received[0].0, ReplicaId::from(&sender_node));
    assert_eq!(received[0].1.recipient, bound_recipient);
    assert_eq!(received[0].1.sender, Some(sender_ref));
    let decoded = registry
        .deserialize::<ReplicatorRead>(received[0].1.message.clone())
        .unwrap();
...
fn connector_stop_clears_late_routes_registered_during_shutdown() {
    let _guard = ddata_socket_test_lock();
    let sender_kit = ActorSystemTestKit::new("ddata-tcp-peer-connector-late-route-sender").unwrap();
    let receiver_kit =
        ActorSystemTestKit::new("ddata-tcp-peer-connector-late-route-receiver").unwrap();
    let retry_interval = Duration::from_millis(25);
    let receiver_port = unused_port();
    ...
    let cluster = Cluster::new(publisher.clone());
    let snapshots = sender_kit
        .create_probe::<ReplicatorTcpPeerConnectorSnapshot>("late-route-snapshots")
        .unwrap();
    ...
    assert_eq!(snapshot.active_targets[0].node(), &receiver_node);
    let initial_address =
        RemoteAssociationAddress::new("kairo", "ddata-connector-initial", "127.0.0.1", Some(2552))
            .unwrap();
    let late_address =
        RemoteAssociationAddress::new("kairo", "ddata-connector-late", "127.0.0.1", Some(2553))
            .unwrap();
