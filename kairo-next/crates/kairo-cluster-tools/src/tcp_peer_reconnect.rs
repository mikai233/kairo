...
/// Result of validating a cluster-tools TCP peer reconnect policy.
pub type ClusterToolsTcpPeerReconnectResult<T> = Result<T, ClusterToolsTcpPeerReconnectError>;

#[derive(Debug, Clone, PartialEq, Eq)]
/// Fixed-interval reconnect policy for membership-derived cluster-tools peers.
...
#[derive(Debug, Clone, PartialEq, Eq)]
/// One cluster-tools peer waiting for a future reconnect attempt.
...
///
/// Time is supplied by the owning runtime, which keeps this policy independent
/// of actor timers and makes due-target selection deterministic.
...
impl ClusterToolsTcpPeerReconnectState {
    ...
    ///
    /// Repeated failures increment the attempt count with saturation and
