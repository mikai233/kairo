...
fn bootstrap_keeps_route_and_clears_pending_reconnect_when_peer_leaves_membership() {
    let _guard = bootstrap_socket_test_lock();
    let sender_kit =
        ActorSystemTestKit::new("cluster-tools-bootstrap-mixed-shrink-sender").unwrap();
    let bound_kit = ActorSystemTestKit::new("cluster-tools-bootstrap-mixed-shrink-bound").unwrap();
    let registry = registry();
    let retry_interval = Duration::from_millis(25);
    let sender_runtime = bind_runtime(
        "cluster-tools-bootstrap-mixed-shrink-sender",
        1,
        11,
        &sender_kit,
        registry.clone(),
    ...
    let missing_port = unused_port();
    let sender_node = sender_runtime.self_node().clone();
    let bound_node = node("cluster-tools-bootstrap-mixed-shrink-bound", bound_port, 2);
    let missing_node = node(
        "cluster-tools-bootstrap-mixed-shrink-missing",
        missing_port,
        3,
    );
    let (bound_runtime, bound_probes) = bind_association_runtime_on_port_with_probes(
        "cluster-tools-bootstrap-mixed-shrink-bound",
        2,
        22,
        bound_port,
        &bound_kit,
